...
def rewrite_all_tabs():
    with open('index.html', 'r', encoding='utf-8') as f:
        html = f.read()

    # Base structure for right-side top-controls
    def build_top_controls(id_attr, class_attr, right_content):
        return f"""<div class="{class_attr}" {id_attr} style="display: flex; flex-direction: row; gap: 12px; margin-bottom: 12px; align-items: flex-end; position: relative; z-index: 10;">
...
<label class="dd-item" data-warehouse="Rau"><input type="checkbox" checked data-col="8" data-warehouse="Rau" onchange="toggleColCheck(8,this)"> Tote đỏ bánh tươi</label>
    ...
    """
    tk_html = build_top_controls("", "top-controls", tk_filters)

    # ==========================
    # 2. Thu Hồi (panel-thuhoi)
    # ==========================
    # Row 1: Filters + Excel
    # Row 2: Stats Block + Legend + Date Bar
    th_filters = """
        <div style="display: flex; flex-direction: row; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="filter-group">
                <span class="filter-label">Kho</span>
...
<!-- CAL_GRID_END -->
    ...
    """
    th_html = build_top_controls('id="thuhoi-controls"', 'top-controls-thuhoi', th_filters)

    # ==========================
    # 3. Sơ Đồ (map-panel)
    # ==========================
    # Row 1: Location, Doanh thu, Tìm kiếm ST, Lân cận, Xuất Excel
    # Row 2: Đo KC (Điểm A, Điểm B, Tính KC)
    # Row 3: Legend + Stats Block
    map_filters = """
        <div style="display: flex; flex-direction: row; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="filter-group">
            ...
            <div class="filter-group">
                ...
                <select class="filter-select" id="map-rev-filter" onchange="filterMapStore()">
                    <option value="">Tất cả</option>
                    <option value="A">Mức A (>4B)</option>
                    <option value="B">Mức B (2-4B)</option>
                    <option value="C">Mức C (<2B)</option>
                </select>
            </div>
            <div class="filter-group">
                <span class="filter-label">Tìm kiếm ST</span>
                <div class="search-wrap">
                    <input class="filter-select search-upper" type="text" id="map-store-search" placeholder="Gõ mã ST, Enter để thêm (tối đa 10)..." oninput="this.value=this.value.toUpperCase();">
            ...
            <button class="btn-export" onclick="resetMapZoom()" style="align-self:flex-end;padding:6px 12px;font-size:11px;height:33px;margin-bottom:0;">📥 Xuất Excel</button>
        </div>
        <div style="display: flex; flex-direction: row; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="filter-group">
            ...
            <button class="btn-export" onclick="calcDistance()" style="align-self:flex-end;padding:6px 12px;font-size:11px;height:33px;margin-bottom:0;">📏 Tính KC</button>
        </div>
        <div style="display: flex; flex-direction: row; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="color-legend" style="margin-bottom: 0;">
            ...
            <div style="flex: 1;"></div>
            <div class="stats-bar compact" style="justify-content: flex-start; gap: 12px; margin-bottom: 0;">
                <div class="stat blue"><div class="stat-num" id="map-total-st">300</div><div class="stat-label">Tổng ST hiển thị</div></div>
                <div class="stat yellow"><div class="stat-num" id="map-total-mart">28</div><div class="stat-label">Mart</div></div>
                <div class="stat green"><div class="stat-num" id="map-total-mini">272</div><div class="stat-label">Mini</div></div>
            </div>
        </div>
    """
    map_html = build_top_controls("", "map-controls", map_filters)


    # ==========================
    # 4. Công Việc (panel-tasks)
    # ==========================
    # Row 1: Filters + Excel
    # Row 2: Stats Block + Date Bar
    task_filters = """
        <div style="display: flex; flex-direction: row; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="filter-group">
            ...
            <div class="filter-group">
                <span class="filter-label">Tìm kiếm</span>
                <div class="search-wrap">
                    <input class="filter-select search-upper" type="text" id="task-search" placeholder="Tiêu đề, ST..." oninput="this.value=this.value.toUpperCase();filterTasks()">
                    <button class="search-clear" onclick="clearSearch('task-search','filterTasks')">&times;</button>
                </div>
            </div>
            <button class="btn-export" onclick="openAddTaskModal()" style="align-self:flex-end;padding:6px 12px;font-size:11px;height:33px;margin-bottom:0;">+ Thêm CV</button>
        </div>
        <div style="display: flex; flex-direction: row; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
            <div class="stats-bar compact" style="justify-content: flex-start; gap: 12px; margin-bottom: 0;">
            ...
            <div style="flex: 1;"></div>
            <div class="cal-bar" style="margin-bottom: 0;" id="task-date-bar">
    ...
    """
    task_html = build_top_controls('id="task-controls"', 'top-controls-tasks', task_filters)

    # 5. Apply
    # We will just replace from `<div class="top-controls..."` up to the closing `</div>` right before `<div class="table-wrap"` or `<div class="map-wrap"` or whatever is next.
    ...
    # Tasks
    task_pat = re.compile(r'<div class="top-controls-tasks"[^>]*>.*?</div>\s*</div>\s*</div>', re.DOTALL)
    if task_pat.search(html):
        html = html[:task_pat.search(html).start()] + task_html + html[task_pat.search(html).end():]
