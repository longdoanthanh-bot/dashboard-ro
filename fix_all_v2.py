...
def fix():
    ...
    # Precise end of map controls
    sm_hf_block = idx[storemap_header_start:storemap_header_end]
    # We want to place filter_row and map_legend after map-controls block
    # map-controls has a closing </div>. In fb12a2d, map-controls ends with </div>, then storemap-header-filters ends with </div>.
    ...
    stats_row_end = tsk.find('</div>', tsk.find('</div>', tsk.find('</div>', tsk.find('stat-right-group')))) # This is terrible.
    # Let's find exactly the end of stats-row by looking for the next sibling: <div class="filter-tabs">
